...
2-  Always correct and retain the following words exactly as written:
    PIN, Allied Bank, CreditCard, DebitCard, Information, Platinum, ABL, Branch, Online Portal, Transaction, etc.

3-  Always spell Allied Bank Limited exactly as it is.

4-  Do not include any person's name.
"""
...
# Language Handling:
#     ->  If the Transcript is in Urdu -> Convert into Roman Urdu (Urdu written in english).
#     -> Otherwise Provide Transcript in english.
...
# 2-  Always correct and retain the following words exactly as written:
#     TPIN, Allied Bank, CreditCard, DebitCard, Information, Platinum, ABL, Branch, Online Portal, Transaction, etc.

# 3-  Always spell Allied Bank Limited exactly as it is.

# 4-  Do not include any person's name.

# 5-  In Transcript, Speaker 1 always name as *Company Representative* and Speaker 2 as *Customer*, means replace all Speaker 1 tag with *Company Representative* and Speaker 2 tag with *Customer*. Provide Result in JSON format as mentioned below.
...
# transcript_Diarization = """Your task is to diarize the transcript of a call between a company representative and a customer. Do not assign specific names; instead, use the tags ' Company Representative' and 'Customer'. 

# Representative mostly starts the Conversation. Hint: The information provider, explains ABL Products, problem solver, or helper is the representative.

# Customers are generally those who are seeking information, looking for solutions to their problems, or inquiring about ABL products and services. They may also need assistance with specific issues.

# Ensure that you accurately segment/label the transcript into these two roles based on the conversation's context. """


# transcript_Diarization = """You are an Expert in Transcript Diarization / Labelization. Your task is to diarize transcript of a call between a *Company Representative* and a *Customer*.
...
# Carefully analyze the context of each utterance and accurately segment the transcript using these two roles. Donot add any unnesscary "\n" as, only add for separating the Representative and Customer segments, except that no other "\n"
# """
transcript_Diarization = """
You are an Expert in Transcript Diarization / Labelization. Your task is to diarize transcript of a call between a *Company Representative* and a *Customer*.

Do not assign personal names. Instead, use the tags "Company Representative" and "Customer". You have to figure out wisely which Transcript segment belongs to *Company Representative* and *Customer* respectively. 

Provide Complete Transcript, Donot miss any part of Transcript. Assign smaller segments to each speaker, one speaker can have multiple segments as well. Further, *\n* in Transcript can help you in identifying the segments.
...
- Correct any labeling mistakes, fix grammar and spelling, and ensure the conversation flows naturally.
...
# Accuracy is crucial, so maintain a high level of contextual understanding while assigning parts of the transcript to the appropriate speaker.

# transcript_Diarization = """
# Your task is to diarize the transcript of a call between a company Representative and a Customer.

# Instructions:
# 1- Label the speakers only as:

#     -> representative
#     -> customer
    
# 2- Do not include any person’s name in the output.

# 3- Most of the time, the conversation is initiated by the representative.
#    (Hint: The information provider, problem solver, or helper in the conversation is usually the representative.)

# 4- Accurately assign each part of the transcript to either representative or customer based on the context of the    conversation.
...
transcript_Summarization = """You are a transcript summarizer. Summarize the following transcript in the same language as the input, and correct any spelling mistakes. Provide a concise and accurate summary in a single paragraph, without including the word 'summary' in the text.
    ...
    For example, your output should look like this (Note: Its just an example for your understanding.):
...
[myABL DigitalBanking, Credit Card, Debit Card, Branchless Banking, Branch/ATM, chequebook, Phone Banking, Asset Management, Transactions, Current Account, Saving Account, Other]
